...
class Event(models.Model):  # thong tin su kien
    ...
    location = models.CharField(max_length=1000, blank=True, null=True)  # dia diem to chuc
    ...
    def update_popularity(self):
        ticket_count = Ticket.objects.filter(ticket_classes__event=self).count()
        # like_count = self.like_set.count()
        # comment_count = self.comment_set.count()
        self.popularity_score = ticket_count * 1.5
        #+ like_count + comment_count * 0.5
        self.save()
...
class Ticket(models.Model):
    ticket_classes = models.ManyToManyField(TicketClass, related_name='tickets')
    user = models.ForeignKey(User, on_delete=models.CASCADE)  # Người mua vé
    ticket_code = models.CharField(max_length=100, unique=True)
    price_paid = models.DecimalField(max_digits=10, decimal_places=2, blank=True)
    is_checked_in = models.BooleanField(default=False)
    checkin_time = models.DateTimeField(null=True, blank=True)
    booked_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.ticket_code} - {', '.join([tc.name for tc in self.ticket_classes.all()])}"
...
class Reservation(models.Model):
    ...
    def __str__(self):
        return f"Reservation {self.id} - {self.ticket_class.name} ({self.quantity})"
...
class PaymentLog(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    ticket_classes = models.ManyToManyField(TicketClass, related_name='paymentlogs')
